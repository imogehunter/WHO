...
{
  //echo("in php");
  $sql_select='SELECT userCountSanchez FROM whoTable WHERE pieceID =1';
  $result = $db->query($sql_select);
  if (!$result) die("Cannot execute query.");

 $row = $result->fetchArray(SQLITE3_ASSOC);
  // note the result from SQL is ALREADy ASSOCIATIVE
 $value  = $row["userCountSanchez"];
 $value = $value+1;

 $sql_update="UPDATE whoTable SET userCountSanchez = '$value' WHERE pieceID =1";
