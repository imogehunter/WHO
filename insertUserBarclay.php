...
{
  //echo("in php");
  $sql_select='SELECT userCountBarclay FROM whoTable WHERE pieceID =1';
  $result = $db->query($sql_select);
  if (!$result) die("Cannot execute query.");

 $row = $result->fetchArray(SQLITE3_ASSOC);
  // note the result from SQL is ALREADy ASSOCIATIVE
 $value  = $row["userCountBarclay"];
 $value = $value+1;

 $sql_update="UPDATE whoTable SET userCountBarclay = '$value' WHERE pieceID =1";
